...
int main(void) {
    ...
    Generate_random_vector(x, n, "x");
    Generate_random_vector(y, n, "y");

    // Imprimir los primeros y últimos 10 elementos de x e y
    Print_vector(x, 10, "First 10 elements of x");
...
void Generate_random_vector(double a[], int n, char vec_name[]) {
    int i;
    printf("Generating random vector %s\n", vec_name);
    srand(time(NULL));  // Inicializa la semilla aleatoria

    for (i = 0; i < n; i++) {
        a[i] = (double)rand() / RAND_MAX;  // Números aleatorios en el rango [0, 1)
    }
}
